...
12. **`recovery_diagnostics` is finalize-filtered.** `RecoverToScopedMax` pushes one diagnostic per call, but the loop's EOF-exit iteration's recovery `Any(1)` may fail and backtrack the recovery capture out — leaving a phantom diagnostic with no surviving capture. `finalize_recovery_diagnostics` (in `src/pegvm/vm.rs`) drops phantoms by checking that each diagnostic's `capture_index` points at a one-byte capture starting at `pos`. Future changes to the `*^` lowering must preserve this shape or update the filter.
