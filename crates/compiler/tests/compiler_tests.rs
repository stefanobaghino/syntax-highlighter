...
/// Workspace root, where `grammars/` lives. `CARGO_MANIFEST_DIR` is the
/// compiler crate (two levels deep at `crates/compiler/`); the
/// workspace-relative directories sit two levels up.
fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(|p| p.parent())
        .expect("compiler crate sits at <workspace>/crates/compiler")
        .to_path_buf()
}
...
fn recover_repeat_labeled_interns_author_label() {
    ...
    let prog = syntax_highlighter_compiler::pegc::compile("root = 'a'*^:bad_thing").unwrap();
    assert_eq!(prog.capture_kinds, vec!["recovery".to_string()]);
...
fn compile_errors_on_uninferable_boundary() {
    ...
    assert!(
        matches!(
            err,
            syntax_highlighter_compiler::pegc::CompileError::CannotInferBoundary { .. }
        ),
...
fn all_shipped_grammars_compile_clean() {
    ...
    for entry in std::fs::read_dir(workspace_root().join("grammars")).expect("read grammars dir") {
...
fn cannot_infer_boundary_carries_placeholder_span_in_display() {
    ...
    match &err {
        syntax_highlighter_compiler::pegc::CompileError::CannotInferBoundary { span, .. } => {
...
fn percent_without_boundary_errors_undefined_rule() {
    ...
    {
        syntax_highlighter_compiler::pegc::CompileError::UndefinedRule(name) => {
...
fn reserved_preferred_conflict_errors() {
    ...
    match err {
        syntax_highlighter_compiler::pegc::CompileError::ReservedPreferredConflict(words) => {
...
fn sibling_scopes_are_isolated() {
    ...
    assert!(
        matches!(err, syntax_highlighter_compiler::pegc::CompileError::UndefinedRule(ref n) if n == "b"),
