//! Grammar source → [`syntax_highlighter::pegvm::Program`] compiler.
//!
//! Symmetric counterpart to [`syntax_highlighter::pegvm`]: `pegvm` *runs* bytecode,
//! `pegc` *compiles* grammar source to bytecode. Depends on `pegvm`
//! (for [`Program`], [`Instruction`](syntax_highlighter::pegvm::Instruction), etc.);
