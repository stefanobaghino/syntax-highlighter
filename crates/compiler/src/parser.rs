//! Deep incremental-parsing abstraction built on [`syntax_highlighter::pegvm`].
//!
...
//! edit point (see [`syntax_highlighter::pegvm::incremental`] for the invalidation
...
impl Parser {
    ...
    /// from [`syntax_highlighter::pegb::decode`], from a hand-built fixture, or from
    ...
    /// [`syntax_highlighter::pegvm::Program::label_kinds`]. Used by diagnostic
    /// consumers to resolve [`syntax_highlighter::pegvm::RecoveryDiagnostic::label`]
    ...
    /// [`syntax_highlighter::pegvm::Program::rule_is_ignore`]. `pegdb recoveries explain`
    ...
    fn reparse(&mut self) {
        ...
        let recovery_kind = self
            ...
            .map(|i| {
                syntax_highlighter::pegvm::CaptureKind(
                    u16::try_from(i).expect("capture_kinds index fits u16"),
                )
            });
