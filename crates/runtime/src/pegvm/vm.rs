...
impl<'p, 'i> VM<'p, 'i> {
    ...
    /// table. Public for tests that need to inspect per-entry details
    /// (notably `examined_max`) and for the public `run_with_cache`
    /// variant that rewraps the table into a
    /// [`MemoCache`](super::incremental::MemoCache). Marked `doc(hidden)`
    /// since the memo-table shape is an internal detail rather than a
    /// stable API.
    #[doc(hidden)]
    pub fn run_core(
        mut self,
