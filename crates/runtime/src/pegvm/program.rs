...
/// [`syntax_highlighter_compiler::pegc`].
#[derive(Debug, Clone)]
pub struct Program {
    pub code: Vec<Instruction>,
    pub capture_kinds: Vec<String>,
    /// Number of rules in the grammar. Each rule gets a distinct `MemoId`
    /// in the range `0..rule_count`, assigned by [`syntax_highlighter_compiler::pegc`] in
    ...
    /// [`syntax_highlighter_compiler::pegc`] in the same order rules receive their `MemoId`s
    ...
    /// [`syntax_highlighter_compiler::pegc`] in the order labels are interned (which is
