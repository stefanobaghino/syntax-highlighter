...
//!
//! Sibling of [`syntax_highlighter_compiler::pegc`]: `pegc` produces a `Program` from grammar
...
mod tests {
    ...
    // Round-trip tests that need a real compiled `Program` (and therefore
    // `pegc`) live in `crates/compiler/tests/pegb_internal_tests.rs`;
    // hosting them here would force a runtime→compiler dependency cycle.
