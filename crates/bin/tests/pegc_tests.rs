...
/// Workspace root, the CWD all subcommands run from. `CARGO_MANIFEST_DIR`
/// is the bin crate (two levels deep at `crates/bin/`); `grammars/` and
/// the per-crate fixture directories sit two levels up.
fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(|p| p.parent())
        .expect("bin crate sits at <workspace>/crates/bin")
        .to_path_buf()
}

/// Verify a workspace-relative path exists, so missing fixtures fail
/// fast rather than letting the binary report a cryptic IO error.
...
fn stats_emits_correct_counts_for_canary_grammar() {
    let path = "crates/compiler/tests/fixtures/stats_canary.peg";
    assert_exists(path);
...
fn stats_per_rule_records_count_references() {
    let path = "crates/compiler/tests/fixtures/stats_refs_canary.peg";
    assert_exists(path);
    let (code, stdout, stderr) = run(&["stats", path]);
    assert_eq!(code, 0, "stderr was: {stderr}");
...
fn stats_grammar_compile_error_exits_3() {
    ...
    let fixture = "crates/compiler/benches/fixtures/medium.json";
